...
//	bool propsLoaded = false;
	...
	private MaterialProperty _DoubleSidedConstants; // vector to describe normal sign

	// Keyword toggles
	private MaterialProperty _DoubleSidedEnable;
	private bool DoubleSidedEnable_Toggle;
	...
	//public readonly String[] GaiaShaderName = new []
	//{
	//	"PWS/PW_General_Forward",
	//	"PWS/PW_General_URP",
	//	"Graphs/PW_General_ShaderGraph_HDRP",
	//	"Graphs/PW_Foliage_ShaderGraph_HDRP"
	//};
	...
	private void  GetShaderType( Shader i_shader )
	{
		...
		case "Shader Graphs/PW_General_ShaderGraph_HDRP":
    		shaderType = GAIA_SHADER_TYPE.HDRP;
			break;
		
		case "Shader Graphs/PW_Foliage_ShaderGraph_HDRP":
	 ...
	 void SetShaderModeProperties( PW_SHADER_MODE i_shadermode)
	 {
			 ...
			 switch (i_shadermode)
			 {
				 case PW_SHADER_MODE.OBJECT_SOLID:
					 targetMat.renderQueue 	= (int)UnityEngine.Rendering.RenderQueue.Geometry + 255;
					 cullMode.floatValue 	= (float)UnityEngine.Rendering.CullMode.Back;
					 PW_Billboard_Toggle = false;
					 AlphaTest_Toggle = false;
					 //HDRP
					 DoubleSidedEnable_Toggle = false;
					 if(_DoubleSidedConstants != null) _DoubleSidedConstants.vectorValue = new Vector4(1,1,1,1);
					 if(_DoubleSidedNormalMode != null) _DoubleSidedNormalMode.floatValue = 2;
					 if (shaderType == GAIA_SHADER_TYPE.HDRP_FOLIAGE)
					 {
						 targetMat.shader = Shader.Find("Shader Graphs/PW_General_ShaderGraph_HDRP");
						 shaderType = GAIA_SHADER_TYPE.HDRP;
					 }
					 break;
				 ...
				 case PW_SHADER_MODE.VEGETATION_CUTOUT:
					 ...
					 //HDRP
					 DoubleSidedEnable_Toggle = true;
					 if(_DoubleSidedConstants != null)  _DoubleSidedConstants.vectorValue = new Vector4(1,1,-1,1);
					 if(_DoubleSidedNormalMode != null) _DoubleSidedNormalMode.floatValue = 1;
					 if (shaderType == GAIA_SHADER_TYPE.HDRP)
					 {
						 targetMat.shader = Shader.Find("Shader Graphs/PW_Foliage_ShaderGraph_HDRP");
						 shaderType = GAIA_SHADER_TYPE.HDRP_FOLIAGE;
					 }
					 break;
	...
	private void FindProperties(MaterialProperty[] props)
	{
		...
		_DoubleSidedConstants = FindPropertySafe("_DoubleSidedConstants", props, targetMat);
		_DoubleSidedEnable = FindPropertySafe("_DoubleSidedEnable", props, targetMat);
		_DoubleSidedNormalMode = FindPropertySafe("_DoubleSidedNormalMode", props, targetMat);

	}

	MaterialProperty FindPropertySafe(string propName, MaterialProperty[] matProp, Material targetMat)
	{
		if (targetMat.HasProperty(propName))
		{
			return FindProperty(propName, matProp);
		}
		else
		{
			return null;
	...
	private void SSSGUIHDRP()
	{
		Header ( "Translucent (SSS) options:" );
		materialEditor.DefaultShaderProperty ( _DiffusionProfile, "Diffusion Profile" );
	}
	...
	public override void OnGUI ( MaterialEditor i_materialEditor, MaterialProperty[] i_properties )
	{
		...
		else if (shaderType == GAIA_SHADER_TYPE.HDRP_FOLIAGE)
		{
			SSSGUIHDRP();
		}
