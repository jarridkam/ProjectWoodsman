...
namespace Gaia.Pipeline.URP
{
    ...
    public static class GaiaURPPipelineUtils
    {
        ...
        private static void ConfigureLighting()
        {
            ...
            {
#if UPPipeline
                Light[] lights = GaiaUtils.FindOOTs<Light>();
                if (lights.Length > 0)
                {
...
#endif
#if UNITY_POST_PROCESSING_STACK_V2
                PostProcessLayer postProcessLayer = GaiaUtils.FindOOT<PostProcessLayer>();
                if (postProcessLayer != null)
                {
                    GameObject.DestroyImmediate(postProcessLayer);
                }

                PostProcessVolume postProcessVolume = GaiaUtils.FindOOT<PostProcessVolume>();
        ...
        private static void UpdateBiomePostFX()
        {
            try
            {
                GaiaPostProcessBiome[] gaiaPostProcessBiomes = GaiaUtils.FindOOTs<GaiaPostProcessBiome>();
        ...
        public static void CleanUpURP(UnityPipelineProfile profile, GaiaSettings gaiaSettings)
        {
            ...
            {
#if UPPipeline
                UniversalAdditionalCameraData[] camerasData = GaiaUtils.FindOOTs<UniversalAdditionalCameraData>();
                GaiaURPRuntimeUtils.ClearUPCameraData(camerasData);

                UniversalAdditionalLightData[] lightsData = GaiaUtils.FindOOTs<UniversalAdditionalLightData>();
        ...
        private static void SetMotionBlurPostFX(bool enabled)
        {
            try
            {
                ...
                PostProcessVolume[] postProcessVolumes = GaiaUtils.FindOOTs<PostProcessVolume>();
        ...
        private static void SetAntiAliasingMode(GaiaConstants.GaiaProAntiAliasingMode antiAliasingMode)
        {
            ...
            {
#if UNITY_POST_PROCESSING_STACK_V2
                PostProcessLayer processLayer = GaiaUtils.FindOOT<PostProcessLayer>();
